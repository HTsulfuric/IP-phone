...
void check_stopper(int ichi) {
  ...
  if (c == '1') {
    write_to_log("log.txt", "Stopper ni yotte process ga shuuryou saremashita!",
                 getpid());
    exit(0);
  }
...
int main(int argc, char **argv) {
  ...
  while (fgets(IP_addr, 20, IP) != NULL) {
    ...
    write_to_log("log.txt", "[initial client] ima kara fork suru yo!",
                 getpid());
    /* forkの分担
    親プロセス: 次のIPアドレスを読み込んで 読み込み終わったら
    serverとしての役割にシフトする 子プロセス:
    読み込んだIPアドレスを使って通話をする
    */
    int qid = fork();
    ...
    if (qid == 0) {
      ...
      write_to_log("log.txt", "[initial client] PS to call hazimeruyo!",
                   getpid());
      // phone serverと通話する
      call(s_to_PS);
  ...
  while (1) {
    ...
    if (bind(socket_to_PC, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
      perror("bind");
      exit(1);
    }
    ...
    write_to_log(
        "log.txt",
        "[phone server] client kiya!! \nima kara PS toshite fork suru yo!",
        getpid());
    int pid = fork();
    if (pid == -1) {
      perror("fork");
    ...
    check_stopper(101);
    if (pid == 0) {
      write_to_log("log.txt", "[phone server] Server toshite call suru yo",
                   getpid());
      call(s);
      check_stopper(6);
    }
